...
public class EidInfo {
     ...
     */
    private void appendTo(StringBuilder builder, String separator,
    ...
    private String formatDate(String date) {
        return date.length() == 8 ? String.format("%s.%s.%s",
                date.substring(0, 2), date.substring(2, 4),
                date.substring(4, 8)) : date;
    }
    ...
    public String getNameFull() {
        return String.format("%s %s %s", get(Tag.GIVEN_NAME),
                get(Tag.PARENT_GIVEN_NAME), get(Tag.SURNAME));
    }

    /**
     * Get place of residence as multiline string. Format parameters can be used
     * to provide better output or null/empty strings can be passed for no
     * special formating.
     * <p/>
     * For example if floorLabelFormat is "%s. sprat" returned string will
     * contain "5. sprat" for floor number 5.
     * <p/>
     ...
     * <p/>
     * FIXME: Use one parameterized format string to allow both
     * "Main street 11" and "11 Main street"
     * <p/>
     * FIXME: Think about how to handle short form format with missing
     * ENTRANCE/FLOOR label (line 298).
     */
    public String getPlaceFull_pdf(String entranceLabelFormat,
                                   String floorLabelFormat, String appartmentLabelFormat) {
        ...
        if (has(Tag.APPARTMENT_NUMBER)) {
            // For appartmentLabel "br. %s" gives
            // "Main street 11 ulaz 2, 5. sprat, br. 4"
            if (has(Tag.ENTRANCE) || has(Tag.FLOOR)) {
                appendTo(out, ", ", appartmentLabelFormat,
                        Tag.APPARTMENT_NUMBER);
            } else {
    ...
    public String getPlaceFull(String entranceLabelFormat,
                               String floorLabelFormat, String appartmentLabelFormat) {
    ...
    /**
     * Get full place of birth as a multiline string, including community and
     * state if present.
     *
     * @return Nicely formatted place of birth as a multiline string.
     */
    public String getPlaceOfBirthFull_pdf() {
        StringBuilder out = new StringBuilder();
    ...
    public Bundle toBundle() {
        ...
        for (Map.Entry<Tag, String> field : fields.entrySet()) {
            result.putString(field.getKey().getKey(), field.getValue());
        }
        //result.putString("place_full",
        //		getPlaceFull("ulaz %s", "%s. sprat", "br. %s"));
        //result.putString("address_date", getAddressDate());
        //result.putString("place_of_birth_full", getPlaceOfBirthFull());
        //result.putString("name_full", getNameFull());
    ...
    public enum Tag {
         ...
         * <p/>
         * While mostly unique, due to the non-bulletproof number allocation
         * scheme, has actually been known to repeat for some rare individuals.
         * The last digit is mod 11 checksum, but due the same reason, there
         * exist numbers with incorrect checksum.
         */
        ...
        /**
         * The parent's given name, the usual 'parenthood' middle name used to
         * disambiguate similarly named persons.
         * <p/>
         ...
         * "Savski Venac"
         */
        COMMUNITY_OF_BIRTH(302, "community_of_birth", "Community of birth"), STATE_OF_BIRTH(
                303, "state_of_birth", "State of birth"), STATE_OF_BIRTH_CODE(
                304, "state_of_birth_code", "State of birth code"), DATE_OF_BIRTH(
                305, "date_of_birth", "Date of birth"),
